...
같은 경로에 있는 모듈을 불러올 때와 다르게  
패키지 내에 있는 모듈을 불러올 때는 `*`를 사용하는 방법이 다르다.
{% highlight py %}
# /util/sub1/sub1_util1.py
def fun1():
    print("util > sub1 > sub_util1.py > fun1 호출")

# main.py
from util.sub1.sub1_util1 import *
fun1() # 오류 발생
{% endhighlight %}

패키지 내의 모듈에 대해서 `*`를 사용하려면  
...
이제 다시 main.py에서 `*`를 통해서 실행해보면  
정상적으로 실행되는 것을 알 수 있다.

TODO : 내용이 다름 => `__all__` 없어도 된다고 나옴 (ChatGPT)
TODO : *를 안 쓰는 게 낫다고 알려주기

TODO:
https://wikidocs.net/1418 => .와 ..에 대해서 작성하기

TODO : pip install 작성하기
