...
- 데이터 조회
    - `XREAD [COUNT 읽을_데이터_개수] [BLOCK 대기_시간] STREAMS key_이름... 아이디...`
        - 지정한 아이디보다 큰 데이터를 읽는다.
            - `XRANGE`처럼 +1을 할 필요가 없다.
            - 반복해서 읽을 때 편하다.
        - `COUNT` 옵션을 사용하면 
        - `BLOCK` 옵션을 사용하면 지정한 시간동안 기다렸다가 데이터를 읽게 할 수 있다.
            - 단위 : 밀리초
        - `XRANGE`와 다르게 키를 여러개 지정할 수 있다.
    - `XLEN key_이름`
        - Streams의 엔트리(ID) 개수를 조회한다.
        - 만약 존재하지 않는 키라면 0을 반환한다.
- 데이터 삭제
    - `XDEL key_이름 아이디...`
        - 데이터를 삭제한다.
        - 여러 개의 아이디를 지정할 수 있다.
        - 삭제된 데이터(ID)의 개수를 반환한다.
-->
