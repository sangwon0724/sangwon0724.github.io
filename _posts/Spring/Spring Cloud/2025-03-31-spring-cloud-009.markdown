...
각 서비스가 단일 인스턴스인 경우에는 위에 코드가 문제가 될 일은 없을 것이다.  
하지만 서비스의 인스턴스가 2개 이상이라면 어떻게 될까?

임베디드 H2말고 MySQL같은 실제로 운영에서 쓸 만한 DB로 생각해보자.  
복제본이나 클러스터 구성같은 것은 신경쓰지 않고  
순수하게 데이터를 저장히기 위해 DB를 1대만 쓴다고 가정해보자.

만약 생산자가 소비자한테 "나 작업했어!"라고  
특정한 토픽에 메시지를 보내게 되면  
소비자는 "메시지가 왔군! 나도 작업을 해야겠다!"라고 반응할 것이다.  
그런데 여기서 소비자의 작업이 데이터 저장이라면  
생산자로부터 받은 데이터를 통해  
소비자는 해당하는 데이터를 인스턴스의 개수만큼 저장하게 될 것이다.

왜냐하면 각 인스턴스는 개별적으로 동작하기 때문이다.  
그러면 AUTO INCREMENT때문에 PK는 다른데 나머지 데이터는 동일한  
중복 데이터가 발생할 수 있다.

#### 해결방법

위와 같은 문제를 해결하려면  
데이터를 DB에 바로 저장하는 것이 아니라  
